...
if(!$_POST['usuario']){
    ...
    $contador = 0; 
      ...
      foreach ($arrayUsuarios as $key => $value) {
        ...
        $directorio = "archivos/".$id_usuario."/".$tipo."/".$ano."/".$mes."/";
            ...
            mkdir("archivos/".$id_usuario."/".$tipo."/".$ano."/".$mes, 0777, true);
                if($contador == 0){
		                    ...
		                    for ($i = 0; $i < $tot; $i++){
                                ...
                                {	          
                                    ...
                                    if(mysqli_query($c,$sql))
                                    {
                                        ...
                                        $cadenAlert = $cadenAlert. "Usuario: ".$id_usuario. " - ERROR DE SUBIDA 1. \\n";
                                    }
		                        }
		                    }
	                }
                    ...
                    if(copy($original, "$directorio/$nombre")){
                        ...
                        $cadenAlert = $cadenAlert. "Usuario: ".$id_usuario. " - ERROR DE SUBIDA 3. por aqui es 2 \\n";
                    }
                }
        }else{
	        if($contador == 0){
                ...
                if (isset ($_FILES["archivo"])) {
		            ...
		            for ($i = 0; $i < $tot; $i++){
		                $tmp_archivo = $_FILES["archivo"]["tmp_name"][$i];
                        $name = $_FILES["archivo"]["name"][$i];
                        $moved = move_uploaded_file($tmp_archivo, "$directorio/$nombre");
			            if($moved)
			            {	   
                            ...
                            if(mysqli_query($c,$sql))
                            {
                                ...
                                $cadenAlert = $cadenAlert. "Usuario: ".$id_usuario. " - ERROR DE SUBIDA 3. \\n";
                            }
		                }
                ...
                if(copy($original, "$directorio/$nombre")){
                    ...
                    $cadenAlert = $cadenAlert. "Usuario: ".$id_usuario. " - ERROR DE SUBIDA 4.  por aqui es 4\\n";
                }
            }                
        }
        $contador++; 
